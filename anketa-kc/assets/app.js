...
// ════════════════════════════════════════════════════════════════════════
// === ROLLBACK START (hardcoded debug defaults) ===
// Старая версия: жёстко прописанные значения для отладки.
// В текущем режиме (вебхук) leadId и currentUser корректно приходят из shim BX24
// (shim читает leadId из URL-параметра ?leadId=..., см. webhook-client.js).
//
// Старый код (оставлен закомментированным для восстановления при необходимости):
// let leadId           = 59466; //null;
// let currentUser      = 14; //null;
// let CURRENT_USERNAME = 'Петряев Денис'; //Было пустое
// ════════════════════════════════════════════════════════════════════════

// Идентификатор лида из Bitrix24 — будет заполнен после инициализации BX24.
// В iframe-режиме: из BX24.placement.info().options.ID.
// В режиме вебхука: из URL-параметра ?leadId=... (обработка внутри webhook-client.js).
let leadId           = null;

// Объект с данными текущего авторизованного пользователя Bitrix24.
// В iframe-режиме: из user.current.
// В режиме вебхука: mock-данные из webhook-client.js.
let currentUser      = null;

// Полное имя текущего пользователя (ФИО) — отображается в шапке формы.
let CURRENT_USERNAME = '';
// === ROLLBACK END (hardcoded debug defaults) ===
