...
- Автоматический polling (периодические запросы расписания) **отключён** — расписание обновляется:
...
│   ├── cities.js      — словарь ~1000 городов России с UTC-офсетами; getCityTZ(name)
│   ├── mp-config.js   — конфигурация МП: город, рабочие часы, дни, длительность слота; getMPDaySlots(), isMPWorkday()
...
- Включает региональные центры, районные города и малые города.
